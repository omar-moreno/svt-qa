...
int main(int argc, char **argv)
{
    ...
    while((option_char = getopt(argc, argv, "i:h:f:un:r:p:s")) != -1){
        switch(option_char){
            case 'i':
    ...
    while(dataRead->next(event)){
        ...
        //cout << "RCE Address: " << event->rceAddress() << endl;
        if(fpga != event->rceAddress()) continue;  
        ...
        for(uint x = 0; x < event->count(); x++){
            ...
            if(samples->head() || samples->tail()) continue; 

            // Skip the sample if it doesn't come from the hybrid of interest
            //cout << "Hybrid ID: " << samples->hybrid() << endl;
    ...
    string title_prefix = "RCE Address: " + PlotUtils::convertToString(fpga) + " Hybrid: " + PlotUtils::convertToString(hybrid);
    ...
    mg_mean_baseline_diff->GetYaxis()->SetLabelSize(0.02); 
