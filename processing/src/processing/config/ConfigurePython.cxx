...
ConfigurePython::ConfigurePython(const std::vector<std::string> &config) { 
  // assumes that nargs >= 0
  //  this is true always because we error out if no python script has been
  //  found

  // Extract the python script name from the path to the script.
  std::string cmd = config[0];
  if (config[0].rfind("/") != std::string::npos) {
    cmd = config[0].substr(config[0].rfind("/") + 1);
  }
  ...
  wchar_t **targs = new wchar_t *[config.size()];
  targs[0] = Py_DecodeLocale(config[0].c_str(), NULL);
  for (int i = 1; i < (config.size() - 1); i++) { 
    targs[i] = Py_DecodeLocale(config[i].c_str(), NULL);
  }
  ...
  PySys_SetArgvEx(config.size(), targs, 1);
  ...
  PyObject *pCMod = PyObject_GetAttrString(script, "SvtQACfg");
  ...
  if (pProcessClass == 0) {
    ...
    EXCEPTION_RAISE(
        "ConfigureError",
        "Process object not defined. This object is required to run svt-qa.");
  }
